...
# Check if technical predictions cluster by batch — can help identify batch effects
...
The counts vector for each sample must match the model's expected number of genes. If the length doesn't match, the API returns a validation error.
...
All count values must be non-negative integers. Floats that are whole numbers (like `10.0`) are accepted, but negative values cause validation errors.
