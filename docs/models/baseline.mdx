...
- `study` — Bioproject ID
- `library_selection` — e.g. `"cDNA"`, `"polyA"`, `"Oligo-dT"` (see the [ENA documentation](https://ena-docs.readthedocs.io/en/latest/submit/reads/webin-cli.html#permitted-values-for-library-selection))
...
Models have a limited acceptable range of metadata input values. If you provide a value that's not in the acceptable range, the API returns an error.
