...
- **`False`** (default) — The output's total count is taken from the reference expression (sum of its counts). Use when you want the synthetic sample to preserve the reference's library size.
...
If `True`, the model uses the mean of each latent distribution (`p(z|metadata)` for perturbation, `q(z|x)` for conditioned components) instead of sampling. Produces deterministic, reproducible outputs.
...
The reference counts vector must match the model's expected number of genes. If the length doesn't match, the API returns a validation error.
