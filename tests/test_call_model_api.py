...
def test_predict_query_live_call_success(test_model_name, test_model_version):
    ...
    if test_model_name == "rMetal" and test_model_version == "v0.2":
        pytest.skip(f"Skipping {test_model_name}{test_model_version}: Not sure what's going on here, but this model/version is consistently failing with 400 Bad Request")
    ...
    if current_model_tuple not in available_model_tuples:
        pytest.skip(f"Model {test_model_name}{test_model_version} not reported as available by the API via get_available_models().")

    try:
        test_query = get_valid_query(test_model_name, test_model_version)
        print("Generated query:", test_query)
    except Exception as e:
        pytest.fail(f"get_valid_query failed for {test_model_name}{test_model_version}: {e}")
    ...
    try:
        ...
        print(f"predict_query call successful for {test_model_name}{test_model_version}.")
    except ValueError as e:
        pytest.fail(f"predict_query for {test_model_name}{test_model_version} raised ValueError: {e}")
    except KeyError as e:
         pytest.fail(f"predict_query for {test_model_name}{test_model_version} raised KeyError (API key issue?): {e}")
    except Exception as e:
        pytest.fail(f"predict_query for {test_model_name}{test_model_version} raised unexpected Exception: {e}")

    assert isinstance(results, dict), f"Result for {test_model_name}{test_model_version} should be a dictionary"
    assert "metadata" in results, f"Result dictionary for {test_model_name}{test_model_version} should contain 'metadata' key"
    assert "expression" in results, f"Result dictionary for {test_model_name}{test_model_version} should contain 'expression' key"

    metadata_df = results["metadata"]
    expression_df = results["expression"]

    assert isinstance(metadata_df, pd.DataFrame), f"'metadata' for {test_model_name}{test_model_version} should be a pandas DataFrame"
    assert isinstance(expression_df, pd.DataFrame), f"'expression' for {test_model_name}{test_model_version} should be a pandas DataFrame"

    assert not metadata_df.empty, f"Metadata DataFrame for {test_model_name}{test_model_version} should not be empty for a valid query"
    assert not expression_df.empty, f"Expression DataFrame for {test_model_name}{test_model_version} should not be empty for a valid query"
...
@pytest.mark.parametrize(
    ...
    [
        (TEST_MODEL_NAME_V0, TEST_MODEL_VERSION_V0, {"modality", "num_samples", "inputs"}, {"output_modality", "mode"}),
    ]
)
...
def test_validate_query_v1_valid():
    ...
    try:
        ...
        print(f"validate_query passed for {V1_MODEL_NAME}{V1_MODEL_VERSION} as expected.")
...
def test_validate_query_v0_valid():
    ...
    try:
        ...
        print(f"validate_query passed for {V0_MODEL_NAME}{V0_MODEL_VERSION} as expected.")
    except (ValueError, TypeError) as e:
