...
Every model on Synthesize Bio accepts specific metadata fields as inputs - things like cell type, tissue, disease state, and perturbation details. The valid values for each field (for example, ontology IDs and cell line names) are defined by the model's vocabulary.
